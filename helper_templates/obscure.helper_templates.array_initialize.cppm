...
namespace obscure::helper_templates
{
    ...
    std::array<T, sizeof...(Idxs)>
    initialize_array_impl(Fcreate creator, std::index_sequence<Idxs...>)
    {
        return std::array<T, sizeof...(Idxs)>{ (Idxs, creator())... };
    }

    export template<typename T, std::size_t Size, typename... TParams>
    std::array<T, Size>
    initialize_array(TParams... Params)
    {
        auto create = [&]() { return T{ Params... }; };
