...
export namespace obscure::vulkan
{
    ...
    struct draw_call_collection<index, TPipeline, TPipelines...>
        : draw_calls_impl<index, TPipeline>
        , draw_call_collection<index + 1, TPipelines...> {
    };
    ...
    struct command_session final : draw_call_collection<0, TPipelines...> {
      private:
        ...
        [[nodiscard]] std::span<vk::Pipeline>
        get_all_pipelines() const final
        {
            return pipeline_refs;
        }

        [[nodiscard]] std::span<vk::DescriptorSetLayout>
        get_all_uniform_descriptor_layouts() const final
        {
            return uniform_descriptor_set_layouts;
        }

        [[nodiscard]] std::span<vk::PipelineLayout>
        get_all_layouts() const final
        {
            return pipeline_layout_refs;
        }

        [[nodiscard]] std::span<vk::DescriptorSetLayout>
        get_all_texture_descriptor_layouts() const final
        {
      ...
      public:
        command_session(vk::CommandBuffer _command_buffer,
                        ...
                        pipeline_collection<sizeof...(TPipelines)>& pipeline_collection)
            ...
            , texture_descriptor_set_layouts(pipeline_collection.texture_descriptor_layouts)
        {
            ...
            vk::RenderPassBeginInfo render_info{ swap_chain.get().render_pass,
        ...
        get_frame_index() const final
        {
            return frame_index;
        }

        [[nodiscard]] vk::Extent2D
        get_extent() const final
        {
            return extent;
        }

        [[nodiscard]] vk::CommandBuffer
        get_command_buffer() const final
        {
