    ...
    struct shader_set : private vk::Device {
      ...
      public:
        ...
        explicit shader_set(vk::Device device)
            : vk::Device(device)
            , shaders{ shader_module::load_shader<Keys>(device)... }
        {
        }
        ...
        get_sub_set() const
        {
            return std::array<vk::ShaderModule, sizeof...(SubKeys)>{ static_cast<vk::ShaderModule>(
                shaders[shader_list::template get_index<SubKeys>()])... };
        }

        template<auto... SubKeys>
        auto
        get_shaders(obscure::key_set<SubKeys...> obj) const
        {
