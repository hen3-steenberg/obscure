...
namespace obscure::vulkan
{
    ...
    consteval vk::DebugUtilsMessageSeverityFlagsEXT
    get_severity_flags()
    {
    ...
    consteval vk::DebugUtilsMessageTypeFlagsEXT
    get_type_flags()
    {
    ...
    template<vk::DebugUtilsMessageSeverityFlagBitsEXT min_severity>
    constexpr bool
    is_valid_severity(vk::DebugUtilsMessageSeverityFlagBitsEXT const severity) noexcept
    {
    ...
    template<>
    constexpr bool
    is_valid_severity<vk::DebugUtilsMessageSeverityFlagBitsEXT::eVerbose>(
        vk::DebugUtilsMessageSeverityFlagBitsEXT const severity) noexcept
    {
        return true;
    }
    ...
    template<vk::DebugUtilsMessageTypeFlagBitsEXT min_type>
    constexpr bool
    is_valid_type(vk::DebugUtilsMessageTypeFlagsEXT const messageType)
    {
    ...
    struct logger_base {
      private:
        ...
        LogCallback(vk::DebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
      ...
      public:
        logger_base()
        {
            vk::DebugUtilsMessengerCreateInfoEXT const create_info{
                {}, get_severity_flags<min_severity>(), get_type_flags<min_type>(), LogCallback, this
            };
      ...
      protected:
        ...
        LogEvent(vk::DebugUtilsMessageSeverityFlagsEXT Severity,
                 ...
                 const vk::DebugUtilsMessengerCallbackDataEXT& Data) = 0;
    };

    constexpr const char*
    get_message_severity_text(vk::DebugUtilsMessageSeverityFlagsEXT const Severity) noexcept
    {
    ...
    constexpr const char*
    get_message_type_text(vk::DebugUtilsMessageTypeFlagsEXT const Type) noexcept
    {
        using namespace obscure::helper_templates;
    ...
    struct Event {
        ...
        struct LogData {
            ...
            struct Label {
                ...
                explicit Label(vk::DebugUtilsLabelEXT const& lbl)
                    : name(lbl.pLabelName ? lbl.pLabelName : "NO NAME")
                    , color(lbl.color)
                {
                }
            };
            ...
            struct Object {
                ...
                explicit Object(vk::DebugUtilsObjectNameInfoEXT const& obj)
                    ...
                    , name(obj.pObjectName ? obj.pObjectName : "NO NAME")
                {
                }
            };
        ...
        Event(vk::DebugUtilsMessageSeverityFlagsEXT _Severity,
    ...
    struct EventQueue {
        ...
        void
        enqueue(vk::DebugUtilsMessageSeverityFlagsEXT Severity,
                vk::DebugUtilsMessageTypeFlagsEXT Type,
        ...
        std::optional<std::unique_ptr<Event>>
        get_next_event()
        {
            std::lock_guard queue_guard{ queue_mtx };
            if (events.empty())
    ...
    struct console_logger : logger_base<min_severity, min_type> {
      private:
        ...
        LogEvent(vk::DebugUtilsMessageSeverityFlagsEXT Severity,
                 vk::DebugUtilsMessageTypeFlagsEXT Type,
                 const vk::DebugUtilsMessengerCallbackDataEXT& Data) final
        {
            std::cout << get_message_severity_text(Severity) << ", " << get_message_type_text(Type) << " : "
                      << Data.pMessage << "\n";
        }
    };

    using verbose_console_logger =
        console_logger<vk::DebugUtilsMessageSeverityFlagBitsEXT::eInfo, vk::DebugUtilsMessageTypeFlagBitsEXT::eGeneral>;
    ...
    struct json_logger : logger_base<min_severity, min_type> {
        ...
        json_logger()
            ...
            , writer_thread([this]() { ProcessMessages(); })
        {
            json_file << "[\n";
        }
      ...
      private:
        ...
        void
        HandleLogEntry(std::unique_ptr<Event> Entry)
        {
            if (first_entry) {
                json_file << "{\n";
                first_entry = false;
            ...
            for (auto const& label : Entry->data.queue_labels) {
                ...
                json_file << "\t\t\"color\" : [" << label.color[0] << ", " << label.color[1] << ", " << label.color[2]
                          << ", " << label.color[3] << "]\n";
                json_file << "\t}\n";
            }
            json_file << "\t],\n";
    ...
    using verbose_json_logger =
        json_logger<vk::DebugUtilsMessageSeverityFlagBitsEXT::eInfo, vk::DebugUtilsMessageTypeFlagBitsEXT::eGeneral>;

    template<bool Enable,
             vk::DebugUtilsMessageSeverityFlagBitsEXT min_severity,
             vk::DebugUtilsMessageTypeFlagBitsEXT min_type,
             template<vk::DebugUtilsMessageSeverityFlagBitsEXT, vk::DebugUtilsMessageTypeFlagBitsEXT> class... TLoggers>
    ...
    template<vk::DebugUtilsMessageSeverityFlagBitsEXT min_severity,
             vk::DebugUtilsMessageTypeFlagBitsEXT min_type,
             template<vk::DebugUtilsMessageSeverityFlagBitsEXT, vk::DebugUtilsMessageTypeFlagBitsEXT> class... TLoggers>
    using logger_collection =
        logger_collection_impl<obscure::enable_debug_validation(), min_severity, min_type, TLoggers...>;
} // namespace obscure::vulkan
#pragma endregion

obscure::name_list<obscure::enable_debug_validation()>
...
static VKAPI_ATTR vk::Bool32 VKAPI_CALL
InstanceLifetimeCallback(vk::DebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
                         vk::DebugUtilsMessageTypeFlagsEXT messageType,
                         const vk::DebugUtilsMessengerCallbackDataEXT* pCallbackData,
                         void* pUserData)
{
...
vk::Instance
create_instance(const char* app_name, obscure::version app_version)
{
    ...
    if constexpr (obscure::enable_debug_validation()) {
        ...
        vk::StructureChain<vk::InstanceCreateInfo, vk::DebugUtilsMessengerCreateInfoEXT> create_info{
            vk::InstanceCreateInfo{
                {}, &app_info, 1, &debug_layer_name, extension_list.get_count(), extension_list.get_names() },
        ...
        vk::InstanceCreateInfo create_info{
            {}, &app_info, 0, nullptr, extension_list.get_count(), extension_list.get_names()
        };
...
struct ApplicationInstance : vk::Instance {
    ...
    ~ApplicationInstance() noexcept { free(); }
};
using logger_t = obscure::vulkan::logger_collection<vk::DebugUtilsMessageSeverityFlagBitsEXT::eVerbose,
...
void
obscure::initialize(const char* app_name, obscure::version app_version)
{
    if (!ctx.has_value()) {
...
vk::Instance
obscure::get_application_instance()
{
    return ctx->get_instance();
}

void
obscure::free_instance() noexcept
{
    logger.reset();
    ctx.reset();
}